...
ssize_t MessageNetworkManager::sendMessage(std::string content) const {
  ...
  if (socket.send(&dataLength, sizeof(uint32_t), 0) == -1) {
    perror("Error while sending number of bytes occurred");
    return -1;
  }

  int numberOfBytes = socket.send(content.data(), content.size(), 0);
  if (numberOfBytes == -1) {
    perror("Error while sending message occurred");
    return -1;
  }
  return numberOfBytes;
}

ssize_t MessageNetworkManager::receiveMessage(std::string &buffer, bool whatIsGoingOn) const {
  uint32_t dataLength;
